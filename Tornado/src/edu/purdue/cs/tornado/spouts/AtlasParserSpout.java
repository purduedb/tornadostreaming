...
public class AtlasParserSpout extends FileSpout implements BaseTornadoQuerySpout {

	public ArrayList<ArrayList<Double>> locationArray;// = super.listArray;
	public ArrayList<ArrayList<String>> KeywordArray;// = super.KeyArray;

	public AtlasParserSpout(Map spoutConf, Integer initialSleepDuration) {
		super(spoutConf, initialSleepDuration);
		// locationArray=super.listArray;
		// KeywordArray=super.KeyArray;
	}

	public static String SPATIAL_RANGE = "SPATIAL_RANGE"; // 1 [10] 100 500 out
															// of 10000
	public static String KEYWORD_COUNT = "KEYWORD_COUNT"; // 1 [5] 10 20
	public static String TOTAL_QUERY_COUNT = "TOTAL_QUERY_COUNT"; // 1000
																	// [100000]
																	// 1000000
	public static Double spatialRangeVal = 10.0;
	public static Integer keywordCountVal = 4;
	...
	Integer Kindex;// = new Integer(0);
	Integer thepointer; // = new Integer(0);
	Integer arraylistpointer;// = new Integer(0); //added
	// final Properties properties =
	// loadProperties(SpatioTextualConstants.CONFIG_PROPERTIES_FILE);
	...
	public void nextTuple() {
		// super.nextTuple();
		if (i >= AtlasParserSpout.totalQueryCountVal) {
			try {
	...
	private void emitQuery(String line, Integer msgId) {
		...
		i = i + 1;
		// this.collector.emit(new Values(q.getQueryType(), q.getQueryId(),
		// q.getFocalPoint().getX(), q.getFocalPoint().getY(),
		// q.getSpatialRange().getMin().getX(),
		// q.getSpatialRange().getMin().getY(),
		// q.getSpatialRange().getMax().getX(),
		// q.getSpatialRange().getMax().getY(), q.getK(),
		// TextHelpers.convertArrayListOfStringToText(q.getQueryText()),
		// TextHelpers.convertArrayListOfStringToText(q.getQueryText2()),
		// q.getTimeStamp(), q.getDataSrc(), q.getDataSrc2(),
		// q.getCommand(), q.getDistance(), q.getTextualPredicate(),
		// q.getTextualPredicate2(),
		// q.getJoinTextualPredicate(),q.getRemoveTime()
		//
		// ));
		this.collector.emit(new Values(q.getQueryType(), q.getQueryId(), q.getSpatialRange().getMin().getX(),
				q.getSpatialRange().getMin().getY(), q.getSpatialRange().getMax().getX(),
				q.getSpatialRange().getMax().getY(), TextHelpers.convertArrayListOfStringToText(q.getQueryText()),
				q.getTimeStamp(), q.getDataSrc(), q.getCommand(), q.getTextualPredicate(), q.getRemoveTime(),
				q.getComplexQueryText()
	...
	public Query buildQuery(String line, String scId, int keywordCount, String data1, String data2, Double distance,
			QueryType queryType, Double spatialRangeVal, TextualPredicate textualPredicate1,
	...
	public Query buildQueryslow(String line) {
		...
		if (lat < SpatioTextualConstants.minLat || lat > SpatioTextualConstants.maxLat
				|| lon < SpatioTextualConstants.minLong || lon > SpatioTextualConstants.maxLong
		...
		if (queryType.equals(QueryType.queryTextualRange)) {
			...
			((JoinQuery) q).setSpatialRange(
					new Rectangle(xy, new Point(xy.getX() + spatialRangeVal, xy.getY() + spatialRangeVal)));
	...
	public Query buildQuery(String line) {
		try {
			...
			synchronized (arraylistpointer) {
				if (arraylistpointer >= locationArray.size())//super.listArray.size())// 10)//super.listArray.size())
				{
					arraylistpointer = new Integer(0);
				}
			}
			// changed
			synchronized (arraylistpointer) {
				try {
					...
					// ArrayList<Double> myarrlist =
					// (Extractor.read(arraylistpointer)).get(0);//super.listArray.get(arraylistpointer);
					ArrayList<Double> myarrlist = locationArray.get(arraylistpointer);
					if (myarrlist.size() != 2) {
						arraylistpointer++;
						Kindex++;
						return null;
					...
					// Thread.sleep(100);
					System.out.println("myarrlist is :" + myarrlist);
					arraylistpointer++; // This is now updating
					mylat = myarrlist.get(0);
					mylon = myarrlist.get(1);
				}

				catch (Exception e) {
					System.out.println("ListArraySize: " + arraylistpointer);// super.listArray.size());
					// e.printStackTrace();
				} // buildQuery(String line)
			}

			if (mylat < SpatioTextualConstants.minLat || mylat > SpatioTextualConstants.maxLat
					|| mylon < SpatioTextualConstants.minLong || mylon > SpatioTextualConstants.maxLong
					|| (Double.compare(mylat, 0.0) == 0 && Double.compare(mylon, 0.0) == 0)) {

				if (previousLocations.size() > 0) {
					latLong = previousLocations.get(r.nextInt(previousLocations.size()));
					prevLocCount++;
				} else {
					System.out.println("out of bounds of entire space lat lon:" + mylat + "," + mylon + "," + line);
					return null;
				}
			} else {
				latLong = new LatLong(mylat, mylon);
			...
			ArrayList<String> queryText1 = new ArrayList<String>();
			ArrayList<String> textList1;
			synchronized (Kindex) {
				if (Kindex >= KeywordArray.size()) // 10){//super.KeyArray.size()){
				{
					Kindex = new Integer(0);
				}

				// ArrayList<String> textList1 =
				// Extractor.readKeys(Kindex).get(0);//super.KeyArray.get(Kindex);
				textList1 = KeywordArray.get(Kindex);
				System.out.println("textlist contains: " + textList1);
				Kindex++;
			...
			ArrayList<String> textList = TextHelpers.transformIntoSortedArrayListOfString(textContent);
			if (textList1.size() < keywordCountVal) {
				// return null;
				if (previousTextList == null)
					return null;
				textList1.addAll(previousTextList);
			} else {
				previousTextList = textList1;
			}

			for (int j = 0; j < textList1.size(); j++) {

				queryText1.add(textList1.get(j));// r.nextInt(textList.size())));
				// queryText2.add(keywordsArr[keywordsArr.length - i - 1]);
			}
			Integer id = i + selfTaskIndex * totalQueryCountVal;
			Point xy = SpatialHelper.convertFromLatLonToXYPoint(latLong);
	...
	public void open(Map conf, TopologyContext context, SpoutOutputCollector collector) {
		...
		Kindex = new Integer(0);
		thepointer = new Integer(0);
		arraylistpointer = new Integer(0); // added
		try{
		locationArray = Extractor.read();// = super.listArray;
		}
		catch(Exception e){			
		}
		try{
		KeywordArray = Extractor.readKeys();
		}
		catch(Exception e){
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields(SpatioTextualConstants.queryTypeField, SpatioTextualConstants.queryIdField,
				SpatioTextualConstants.queryXMinField, SpatioTextualConstants.queryYMinField,
				SpatioTextualConstants.queryXMaxField, SpatioTextualConstants.queryYMaxField,
				SpatioTextualConstants.queryTextField, SpatioTextualConstants.queryTimeStampField,
				SpatioTextualConstants.dataSrc, SpatioTextualConstants.queryCommand,
				SpatioTextualConstants.textualPredicate, SpatioTextualConstants.removeTime,
				SpatioTextualConstants.queryComplexTextField));
	}
